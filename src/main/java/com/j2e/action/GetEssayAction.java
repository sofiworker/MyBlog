...
@Component
@Log4j2
public class GetEssayAction extends BaseAction<EssayDto> {
    ...
    @Autowired
    public GetEssayAction(EssayService service){
        this.service = service;
    }


    @Action(value = "/getEssay", results = {@Result(type = "json", name = "success", params = {"root","data"}),
            @Result(type = "json", name = "error", params = {"root", "data"})})
    public String search(){
        data.setData(service.getEssay(str));
        return SUCCESS;
    }
}